...
from mmdet.core import eval_recalls, voc_classes
...
class VOCDataset(CustomDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        if not self.test_mode:            
            ...
            if 'box_noise_level' in self.kwargs:
                box_noise_level = self.kwargs['box_noise_level']
                if box_noise_level > 0:
                    ann_name = ann_file.split('/')[-1].split('.')[0]
                    prefix = self.img_prefix + 'noisy_pkl/'
    ...
    def format_extra(self, data):
        ...
        if 'gt_bboxes_real' in data:
            data['img_metas'].data['gt_bboxes_real'] = data['gt_bboxes_real'].data
            data['img_metas'].data['img'] = data['img'].data

...
class EvalHook(Hook):
    ...
    def __init__(self, dataloader, dataloader_t=None, dataloader_n=None, start=None, interval=1, **eval_kwargs):
        ...
        self.dataloader_t = dataloader_t
        self.dataloader_n = dataloader_n
    ...
    def evaluate_t(self, runner, results):
        eval_res = self.dataloader_t.dataset.evaluate(
            results, logger=runner.logger, **self.eval_kwargs)
        for name, val in eval_res.items():
            runner.log_buffer.output[name + '_t'] = val
        runner.log_buffer.ready = True

    def evaluate_n(self, runner, results):
        eval_res = self.dataloader_n.dataset.evaluate(
            results, logger=runner.logger, **self.eval_kwargs)
        for name, val in eval_res.items():
            runner.log_buffer.output[name + '_n'] = val
        runner.log_buffer.ready = True
...
class DistEvalHook(EvalHook):
    ...
    def __init__(self,
                 ...
                 **eval_kwargs):
        super().__init__(
            dataloader, dataloader_t, dataloader_n, start=start, interval=interval, **eval_kwargs)
        self.tmpdir = tmpdir
    ...
    def after_train_epoch(self, runner):
        ...
        if self.dataloader_t is not None or self.dataloader_n is not None:
            results_train = multi_gpu_test(
                runner.model,
                self.dataloader_t if self.dataloader_t is not None else self.dataloader_n,
                tmpdir=tmpdir,
                gpu_collect=self.gpu_collect)
            if self.dataloader_t is not None:
                if runner.rank == 0:
                    print('\n')
                    print('VAL_t:')
                    self.evaluate_t(runner, results_train)
            if self.dataloader_n is not None:
                if runner.rank == 0:
                    print('\n')
                    print('VAL_n:')
                    self.evaluate_n(runner, results_train)
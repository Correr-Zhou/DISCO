...
class ConvFCBBoxHeadDISCO(BBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCBBoxHeadDISCO, self).__init__(*args, **kwargs)
        ...
        # DISCO params  
        self.loss_lambda = kwargs['loss_lambda'] if 'loss_lambda' in kwargs else 0.1
        self.loss_gamma = kwargs['loss_gamma'] if 'loss_gamma' in kwargs else 0.3

        self.iter_assign_flag = kwargs['iter_assign_flag'] if 'iter_assign_flag' in kwargs else True
        self.assgin_time = kwargs['assgin_time'] if 'assgin_time' in kwargs else 2
        self.samples_per_gpu = kwargs['samples_per_gpu'] if 'samples_per_gpu' in kwargs else 8

        self.temper_coef = kwargs['temper_coef'] if 'temper_coef' in kwargs else 0.1
        self.dist_refine_flag = kwargs['dist_refine_flag'] if 'dist_refine_flag' in kwargs else True

        self.cls_calib_flag = kwargs['cls_calib_flag'] if 'cls_calib_flag' in kwargs else True
        self.sample_num = kwargs['sample_num'] if 'sample_num' in kwargs else 10
        self.selected_num = kwargs['selected_num'] if 'selected_num' in kwargs else 1

        # oais params
        self.reg_calib_flag = kwargs['reg_calib_flag'] if 'reg_calib_flag' in kwargs else True
        self.reg_calib_epoch = kwargs['reg_calib_epoch'] if 'reg_calib_epoch' in kwargs else 1
        self.mix_a = kwargs['mix_a'] if 'mix_a' in kwargs else 5
        self.mix_b = kwargs['mix_b'] if 'mix_b' in kwargs else 0.85

        # kl params
        self.variance_flag = kwargs['variance_flag'] if 'variance_flag' in kwargs else True
        ...
        self.var_sigma_t = kwargs['var_sigma_t'] if 'var_sigma_t' in kwargs else 0.02
    ...
    def init_weights(self):
        super(ConvFCBBoxHeadDISCO, self).init_weights()
...
@HEADS.register_module()
class Shared2FCBBoxHeadDISCO(ConvFCBBoxHeadDISCO):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(Shared2FCBBoxHeadDISCO, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
    ...
    def loss(self,
        ...
        if cls_score is not None:
            if cls_score.numel() > 0:
                ...
                losses['acc_pos_ori'] = accuracy(cls_score[pos_inds.type(torch.bool)], labels[pos_inds.type(torch.bool)])
                neg_inds = (labels == self.num_classes)
                losses['acc_neg_ori'] = accuracy(cls_score[neg_inds.type(torch.bool)], labels[neg_inds.type(torch.bool)])

                losses['ratio_pos'] = torch.sum(pos_inds) / labels.shape[0]
                losses['ratio_neg'] = torch.sum(neg_inds) / labels.shape[0]

                pos_prob = torch.softmax(cls_score[pos_inds.type(torch.bool)], dim=1)
                neg_prob = torch.softmax(cls_score[neg_inds.type(torch.bool)], dim=1)
                losses['ratio_pos2bg/pos'] = torch.sum(torch.argmax(pos_prob, dim=1) == self.num_classes) / torch.sum(pos_inds)

                losses['prob_pos_mean'] = torch.mean(pos_prob[torch.arange(pos_prob.shape[0]), labels[pos_inds.type(torch.bool)]])
                losses['prob_neg_mean'] = torch.mean(neg_prob[torch.arange(neg_prob.shape[0]), labels[neg_inds.type(torch.bool)]])
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                if pseudo_bbox_targets is None:
                    ...
                    losses['loss_var'] = bbox_var.sum() * 0
                else:
                    losses['loss_bbox'] = self.loss_bbox(
                        pos_bbox_pred,
                        pseudo_bbox_targets,
                    ...
                    if pseudo_bbox_logvar is not None:
                        losses['loss_var'] = self.loss_gamma * self.loss_bbox(
                            pos_bbox_var,
                            pseudo_bbox_logvar,
                            bbox_weights[pos_inds.type(torch.bool)],
                            avg_factor=bbox_targets.size(0),
        ...
        def nms_class(cls_boxes, nms_iou):
            ...
            while cls_boxes.shape[0] > 0:
                ...
                if variance is not None:
                    ...
                    pred_bboxes = kl_bboxes[:, :4]
                    w = (pred_bboxes[:, 2] - pred_bboxes[:, 0]).view(-1, 1)
                    h = (pred_bboxes[:, 3] - pred_bboxes[:, 1]).view(-1, 1)
